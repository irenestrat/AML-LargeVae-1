...
from utils_MoG import *

class VAE(nn.Module):

    def __init__(self, use_gpu, model_path, data_shape, latent_length, dataset_name, number_of_gaussians):
        ...
        self.meanP, self.sigmaP = get_guassian_parameters(self.number_of_gaussians)
        self.meanP = torch.from_numpy(self.meanP).detach()
        self.sigmaP = torch.from_numpy(self.sigmaP).detach()
        if self.use_gpu:
            self.meanP = self.meanP.cuda()
            self.sigmaP = self.sigmaP.cuda()
        ...
        self.encoder_latent_mean = nn.Linear(300, self.latent_length*self.number_of_gaussians)
        self.encoder_latent_sigma = nn.Linear(300, self.latent_length*self.number_of_gaussians)
    ...
    def forward(self, x):
        ...
        ### expanding means and sigmas to a new dimensions based on the number of gaussians
        latent_mean = latent_mean.reshape([latent_mean.shape[0], int(latent_mean.shape[1] / self.number_of_gaussians), self.number_of_gaussians])
        latent_sigma = latent_sigma.reshape([latent_sigma.shape[0], int(latent_sigma.shape[1] / self.number_of_gaussians), self.number_of_gaussians])

        if self.use_gpu:
            eps = torch.normal(0, 1, size=latent_sigma.size()[:2]).cuda()
        else:
            eps = torch.normal(0, 1, size=latent_sigma.size()[:2])
        ## MoG
        z_priors = []
        for index in range(self.number_of_gaussians):
            # author: Ioannis
            #### Sampling z_prior ##
            std = torch.exp(0.5 * latent_sigma[:, :, index])  # log(s^2) = 2log(s) : logvariance -> s=exp[0.5*2log(s)]
            z_priors.append((eps * std + latent_mean[:, :, index])/self.number_of_gaussians)  ## reparameterization trick
            ## might be implemented with learnable coefs ##
            coef = 1/self.number_of_gaussians
            if index > 0:
                z_prior += coef*z_priors[-1]
            else:
                z_prior = coef*z_priors[-1]
        ...
        return output_mean, output_sigma, z_priors, latent_mean, latent_sigma
    ...
    def train_model(self, data_loader, optimizer, KL_coef=1):
        ...
        for index, data_batch in enumerate(data_loader):
            ...
            output_mean, output_sigma, z_priors, latent_mean, latent_sigma = self.forward(data_batch)
            ### computing ELBO-loss
            ## KL divergence
            for index, z_prior in enumerate(z_priors):
                ...
                log_z_prior = torch.sum(-0.5 * (self.sigmaP[index] + torch.pow(z_prior - self.meanP[index], 2) / torch.exp(self.sigmaP[index])), 1) ## loglikelihood of z_prior being generated by

                log_q_z = torch.sum(-0.5 * (latent_sigma[:, :, index] + torch.pow(z_prior - latent_mean[:, :, index], 2) / torch.exp(latent_sigma[:, :, index])), 1) ## loglikelihood of z_prior being generated by
                coef=1/self.number_of_gaussians                                                                                                       ## predicted Normal Distributions
                if index > 0:
                    KL_loss -= coef*(log_z_prior - log_q_z) ## KL(q||p)=Sum_over( q(z)*log[q(z)/p(z)])
                else:
                    KL_loss = - coef*(log_z_prior - log_q_z) ## KL(q||p)=Sum_over( q(z)*log[q(z)/p(z)])
    ...
    def val_model(self, epoch, data_loader, KL_coef=1, test_mode=False):
        ...
        for index, data_batch in enumerate(data_loader):
            ...
            ### computing ELBO-loss
            ## KL divergence
            for i, z_prior in enumerate(z_priors):
                # log_z_prior = torch.sum(-0.5 * torch.pow(z_prior, 2), 1) ## loglikelihood of z_prior being generated by (0,1) Normal Distribution ## summing over the
                log_z_prior = torch.sum(-0.5 * (self.sigmaP[i] + torch.pow(z_prior - self.meanP[i], 2) / torch.exp(self.sigmaP[i])), 1)  ## loglikelihood of z_prior being generated by

                log_q_z = torch.sum(-0.5 * (latent_sigma[:, :, i] + torch.pow(z_prior - latent_mean[:, :, i], 2) / torch.exp(latent_sigma[:, :, i])), 1)  ## loglikelihood of z_prior being generated by
                coef = 1 / self.number_of_gaussians  ## predicted Normal Distributions
                if i > 0:
                    KL_loss -= coef * (log_z_prior - log_q_z)  ## KL(q||p)=Sum_over( q(z)*log[q(z)/p(z)])
                else:
            ...
            if index == 0:
                ...
                columns = 4
                rows = 5
                for indexFig, i in enumerate(range(1, columns * rows + 1, 2)):

                    if self.use_gpu:
                        imgGT = data_batch.data.cpu().numpy()[indexFig].reshape(self.data_shape)
                        imgReconstructed = output_mean.data.cpu().numpy()[indexFig].reshape(self.data_shape)
                    else:
                        imgGT = data_batch.data.numpy()[indexFig].reshape(self.data_shape)
                        imgReconstructed = output_mean.data.numpy()[indexFig].reshape(self.data_shape)
